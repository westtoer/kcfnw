...
NewNumbers.prototype.scoreSuggestedNrForKnoop = function (newnr, knoop, params, lenient) {
    ...
    var me = this, sameNrSet, minDistSame, linkedByNrs, friendSet, minDistFriend,
        gain = 0, score = 0, scoreMsg, failMsg, failReason,
        WEIGHT = {"USE": 30.0, "SAMEDIST": 20.0, "DIGIT" : 10.0, "FRIENDDIST": 2.50 };
    ...
    if (!lenient && minDistSame < params.minDist) {
        ...
        gain = Math.pow((minDistSame - params.minDist) / params.minDist, 3) * WEIGHT.SAMEDIST;
        score += gain;
        scoreMsg += util.format("samD +(%s),", gain);
    }
    ...
    if (!lenient && minDistFriend < params.minDist * 0.2) { // we are a lot less picky for friends
        ...
        // tune up the score pro rato of exces km
        gain = Math.pow((minDistFriend - params.minDist) / params.minDist, 3) * WEIGHT.FRIENDDIST;
        score += gain;
        scoreMsg += util.format("frnD +(%s),", gain);
    }

    // promote the score with normalised available expected usages for this nr
    gain += Math.pow((params.expectedUse - (sameNrSet.length + 1)) / params.expectedUse, 3) * WEIGHT.USE;
    score += gain;
    scoreMsg += util.format("use +(%s),", gain);
    ...
    knoopSet.forEach(function (knoop) {
        ...
        if (knoop.oldnr > 0
            ...
            if (suggestions.length === 0) {
                //TODO try the sparenums anyway
                dbgFn(util.format(" no normal suggestions going to sparenums for knoop %s, reasons counted ==> %j", knoop, reasons));
                me.log("WARN", util.format("Going to sparenums because there are no normal suggestions found for knoop: %s", knoop));
            ...
            if (suggestions.length === 0) {
                dbgFn(util.format(" no sparenum suggestions found for knoop %s, reasons counted ==> %j", knoop, reasons));
                reasons = {};
                me.log("WARN", util.format("Going to lenient mode because there are no normal suggestions found for knoop: %s", knoop));
                //TODO try all nums, but now request to be lenient
                for (checknr = 1; checknr < 100; checknr += 1) {
        ...
        if (!isNaN(newnr)) {
            dbgFn(" registration for knoop %s @ %d ", knoop, newnr);
...
NewNumbers.processor[ReNum.MAKE] = function (knoopSet, params) {
    ...
    NewNumbers.processor[ReNum.UPDT].call(this, knoopSet, params, "Create", true);
};

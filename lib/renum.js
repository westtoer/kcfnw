...
Input.prototype.readKnoop = function (data) {
    ...
    if (this.knoopByCode.hasOwnProperty(knoop.code)) {
        ...
        if (!contains(['FIX', 'DEL', 'IN', 'OUT'], match.status) || !contains(['FIX', 'DEL', 'IN', 'OUT'], knoop.status)) {
            this.errKnoop(knoop, "Knoop-code already in use. This should never happen for status of match [%s] or knoop [%s]!", match.status, knoop.status);
        }
...
Input.prototype.readLink = function (data) {
    ...
    var values, startCode, endCode, link, checkpid, NOMATCH = {}, match = NOMATCH;
    try {
        link = {
...
Input.prototype.reportCounts = function () {
    ...
    Object.keys(this.knoopByCode).forEach(function (code) {
        ...
        if (countTargets < 3 && !contains(['DEL', 'EXT'], knoop.status)) {
...
NewNumbers.prototype.processKeepSet = function (knoopSet, minDist) {
    ...
    knoopSet.forEach(function (knoop) {
        if (knoop.status !== 'OUT' && knoop.status !== 'EXT') {
            if (knoop.status !== 'FIX') {
...
function renumber(input) {
    ...
    ReNum.ACTIONS.forEach(function (action) {
        nn.process(action, input.knoopSet[action], input.rectpos.reuseGraceDist);
    });

...
Input.prototype.readKnoop = function (data) {
    ...
    if (knoop.status === "NEW") { // new
        ...
        if (knoop.codeval < this.cutoffval) {
            this.errKnoop(knoop,
                          util.format("knoop in status NEW should have code beyond '%s'(%d)", this.cutoffcode, this.cutoffval));
        }
    } else { //existing
...
Input.prototype.readLink = function (data) {
    ...
    if (isEmptyCode(data.STARTCODE) || isEmptyCode(data.ENDCODE)) {
        if (link.status !== 'NIV') {
            this.errLink(link, util.format("no start or end-code for knoop '%s' --> '%s'", data.STARTCODE, data.ENDCODE));
        }
    ...
    if (this.linkByCode.hasOwnProperty(link.code)) {
        ...
        if (match.pid !== link.pid || match.start !== link.start || match.end !== link.end) {
            this.errLink(link, util.format("matching link for code does not match pid (%s) or start '%s' end '%s'", match.pid,
    ...
    if (link.status !== 'NEW') {
        ...
        if (checkpid !== link.pid) {
            this.errLink(link, util.format("pid doesn't match the start-stop id's [%s]", checkpid));
        } else {
...
Input.prototype.reportCounts = function () {
    ...
    Object.keys(this.knoopByCode).forEach(function (code) {
        ...
        me.histKnoopTargets[countTargets] = me.histKnoopTargets[countTargets] || [];
        me.histKnoopTargets[countTargets].push(knoop);

        if (knoop.status === 'NEW') {
            me.newKnoop.push(code);

            // then extra-test -- all links must have status NEW!
            Object.keys(knoop.targets.forEach(function (lc) {
                var link = knoop.targets[lc].link;
                if (link.status !== 'NEW') {
                    me.errLink(link, util.format("Link should be new if one of its endpoints is new: '%s'", code));
                }
            }));
            ...
            me.keepKnoopByVolgnr[knoop.oldnr] = me.keepKnoopCodeByVolgnr[knoop.oldnr] || [];

...
Input.prototype.readKnoop = function (data) {
    ...
    if (knoop.status === 'TODO') {
        this.errKnoop(knoop, "knoop is still todo!");
        return; // no need to continue
    }

    if (this.knoopByCode.hasOwnProperty(knoop.code)) {
        match = this.knoopByCode[knoop.code];
        if (!containsAll(['FIX', 'DEL', 'IN', 'OUT'], [match.status, knoop.status])) {
            this.errKnoop(knoop,
    ...
    if (contains(['IN', 'OUT', 'FIX', 'DEL'], knoop.status)) {
    ...
    Object.keys(this.knoopByCode).forEach(function (code) {
        ...
        if (knoop.status === 'FIXNEW') {
            Object.keys(knoop.targets).forEach(function (tgtCode) {
                ...
                hasExtTarget = hasExtTarget || (tgt.status === 'EXT' || tgt.status === 'NEW');
            });

            if (!hasExtTarget) {
                me.errKnoop(knoop, "Knoop in status FIXNEW should at least have one target in NEW or EXT");
            }
        }
        ...
        } else if (contains(['FIXNEW', 'EXT'], knoop.status)) {
...
NewNumbers.prototype.register = function (newnr, knoop, minDist, spareNums) {
    var me = this, sameNrSet;

    newnr = Number(newnr);
    if (contains(spareNums, newnr)) {
        me.log("ERR", util.format("should not register knoop %s for num %d in sparenum %j", knoop, newnr, spareNums));
    }
...
NewNumbers.processor[ReNum.KEEP] = function (knoopSet, minDist, spareNums) {
    var me = this;
    console.log("first   get through the ones to {keep} #=%d", knoopSet.length);
    knoopSet.forEach(function (knoop) {
        if (!contains(['OUT', 'EXT'], knoop.status)) {
            if (!contains(['FIX', 'FIXNEW'], knoop.status)) {
                throw util.format("unexpected status for a knoop that should be kept -- %s", knoop);
            } else {
                me.register(knoop.oldnr, knoop, minDist, spareNums);
            }
        } else {
            // don't register the ones 'ext' or 'out', but do keep the oldnr as newnr for link tracking
...
NewNumbers.processor[ReNum.UPDT] = function (knoopSet, minDist, spareNums) {
    console.log("then    get through the ones to {updt} #=%d", knoopSet.length);
};

NewNumbers.processor[ReNum.MAKE] = function (knoopSet, minDist, spareNums) {
...
NewNumbers.prototype.process = function (action, input) {
    ...
    NewNumbers.processor[action].call(this, knoopSet, minDist, spareNums);
};

...
NewNumbers.prototype.scoreSuggestedNrForKnoop = function (newnr, knoop, params) {
    var me = this, sameNrSet, minDistSame, linkedByNrs, friendSet, minDistFriend, score = 0, scoreMsg, failMsg;
    ...
    if (minDistSame < params.minDist) {
        // bail out here
        return {"score": NaN, "failMsg" : util.format("No reuse of nr %d for samedist == %s", newnr, minDistSame)};
    } else if (minDistSame !== undefined) {
        score += (minDistSame - params.minDist) / 3; // tune up the score pro rato of exces km
        scoreMsg += util.format("samD +(%s),", (minDistSame - params.minDist) / 3);
    }
        ...
        if (!isNaN(score)) { // only if we haven't received a fail yet
            if (!isNaN(tgt.newnr) && tgt.newnr !== -1) { // de target is already known
                ...
                if ((Math.floor(tgt.newnr / 10) === Math.floor(newnr / 10)) ||  (tgt.newnr % 10 === newnr % 10)) {
                    score += 10;
                    scoreMsg += util.format("dgtS{%d} +(10.0),", tgt.newnr);
                }
            }
        }
    });
    // bailout if needed
    if (isNaN(score)) { return {"score": NaN, "failMsg" : failMsg};  }
    ...
    if (minDistFriend < params.minDist * 0.2) { // we are a lot less picky for friends
        // bail out here
        return {"score": NaN, "failMsg" : util.format("No reuse of nr %d for friendist == %s", newnr, minDistFriend)};
    } else if (minDistFriend !== undefined) {
        score += (minDistFriend - params.minDist) / 3; // tune up the score pro rato of exces km
        scoreMsg += util.format("frnD +(%s),", (minDistFriend - params.minDist) / 3);
    }

    // promote the score with 2 times the available expected usages for this nr
    score += ((params.expectedUse - sameNrSet.length + 1) / params.expectedUse) * 10;
    scoreMsg += util.format("use +(%s),", ((params.expectedUse - sameNrSet.length + 1) / params.expectedUse) * 10);
...
NewNumbers.processor[ReNum.UPDT] = function (knoopSet, params, variant) {
    variant = variant || "Update";

    var me = this, preCount, postCount, diffCount;
    ...
    knoopSet.forEach(function (knoop) {
        ...
        var newnr = NaN, suggest, suggestions = [], nr;
        ...
        if (knoop.oldnr > 0
            ...
            newnr = knoop.oldnr;
            //console.log("+++++DBG+++++ just keep nr %d for knoop %s ", newnr, knoop);
        } else {
            //console.log("+++++DBG+++++ make suggestion list for knoop %s ", knoop);
            // if not build a suggestions-array and take the first
            for (nr = 1; nr < 100; nr += 1) {
                if (!contains(params.spareNums, nr) && nr !== knoop.oldnr) { // do not suggest sparenumbers nor oldnr
                    suggest = me.scoreSuggestedNrForKnoop(nr, knoop, params);
                    if (!isNaN(suggest.score)) {
            ...
            if (suggestions.length > 0) {
                ...
                /*
                console.log("+++++DBG+++++ suggestion list for knoop %s yields %d ", knoop, newnr);
                for (var i = 0; i<3; i++){
                    console.log("+++++DBG+++++ ----  list #%d {nr =%d, score =%s, dbg= %s}", i, suggestions[i].nr,
        ...
        if (!isNaN(newnr)) {
            //console.log("+++++DBG+++++ registration for knoop %s @ %d ", knoop, newnr);

...
NewNumbers.prototype.writeResultCSV = function (knoopByCode, linkByCode, outDir) {
    ...
    Object.keys(linkByCode).forEach(function (lnkcode) {
        ...
        if (link.status === 'NEW') {
            newstat = "FIX";
        } else if (link.status === 'FIX') {
            newstat = "FIX";
        } else if (link.status === 'MIS') {
            newstat = "FIX";
        } else if (link.status === 'NIV') {
            newstat = "DEL";
        ...
        links.push([ link.FID, link.pid, link.code, link.start.code, link.end.code, minnr, maxnr,

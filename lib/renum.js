...
NewNumbers.prototype.reportUsage = function (s) {
    ...
    for (nr = 1; nr < 100; nr += 1) {
        ...
        s.write(util.format("%s  - used %d - @dist >%s< - Linked To: [%s]", nr, cnt, mindist, friendnrs));
        s.write("\n");
    }
...
NewNumbers.processor[ReNum.TRKP] = function (knoopSet, minDist, spareNums) {
    var me = this;
    console.log("NewNum **2** get through the ones to {trkp} #=%d", knoopSet.length);
    knoopSet.forEach(function (knoop) {
        console.log("TODO - TryKeep number for knoop %s", knoop);
    });

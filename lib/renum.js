...
function readKnoop(tpl, data) {
    ...
    if (tpl.knoopByCode.hasOwnProperty(knoop.code)) {
        matchingKnoop = tpl.knoopByCode[knoop.code];
        if (matchingKnoop.id !== knoop.id || matchingKnoop.oldnr !== knoop.oldnr) {
            errKnoop(tpl, knoop, "matching knoop for code does not match id and oldnr");
            return;
        }
        matchingKnoop.netwerken.push(knoop.netwerken[0]);
        knoop = matchingKnoop;
    } else {

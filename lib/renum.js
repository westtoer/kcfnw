...
Input.prototype.readKnoop = function (data) {
    ...
    try {
        knoop = {
            ...
            FID: data.FID, // basic rountrip reference for arcGis
            code: data.CODE,
...
Input.prototype.readLink = function (data) {
    ...
    if (this.linkByCode.hasOwnProperty(link.code)) {
        this.errLink(link, "Link-code already in use. This should never happen !");
        return;
    }
...
NewNumbers.prototype.register = function (newnr, knoop, params) {
    ...
    var me = this, sameNrSet, minDistSame, minDistSameOwn, minDistKnoop;
    ...
    sameNrSet.forEach(function (other) {
        ...
        if (minDistSame === undefined || dist < minDistSame) {
            minDistSame = dist;
            minDistKnoop = other;
            //doublecheck our OWN performance -- if one of the knopen is ours
            if (!contains(['EXT', 'EXTNEW'], knoop.status) || !contains(['EXT', 'EXTNEW'], other.status)) {
                if (minDistSameOwn === undefined || dist < minDistSameOwn) {
                    minDistSameOwn = dist;
                }
            }
        }
    ...
    if (sameNrSet.minDistOwn === undefined || minDistSameOwn < sameNrSet.minDistOwn) {
        sameNrSet.minDistOwn = minDistSameOwn;
    }
...
NewNumbers.prototype.writeResultCSV = function (knoopByCode, linkByCode, outDir) {
    var knopen = [], links = [], map = [], stats = [],
        nr, mapheaders, lnknr, linkedNrs,
        nrset, cnt, minDist, minDistOwn, friends;


    /*-------------------------------  result list of assgigned nrs ---------*/
    Object.keys(knoopByCode).forEach(function (code) {
        var knoop = knoopByCode[code],
            newstat = "ERROR",
            change = (knoop.oldnr === knoop.newnr) ? 0 : 1,
            ok = 0;

        if (knoop.status === 'NEW') {
            ok = (!isNaN(knoop.newnr) && (knoop.newnr > 0) && (knoop.newnr < 100)) ? 1 : 0;
            newstat = "FIX";
        } else if (knoop.status === 'EXT') {
            ok = (knoop.newnr === knoop.oldnr) ? 1 : 0;
            newstat = "EXT";
        } else if (knoop.status === 'EXTNEW') {
            ok = (isNaN(knoop.newnr) || (knoop.newnr === knoop.oldnr)) ? 1 : 0;
            newstat = "EXT";
        } else if (knoop.status === 'FIX') {
            ok = (!isNaN(knoop.newnr) && (knoop.newnr > 0) && (knoop.newnr < 100)) ? 1 : 0;
            newstat = "FIX";
        } else if (knoop.status === 'FIXNEW') {
            ok = (knoop.newnr === knoop.oldnr) ? 1 : 0;
            newstat = "FIXNEW";
        } else if (knoop.status === 'IN') {
            ok = (!isNaN(knoop.newnr) && (knoop.newnr > 0) && (knoop.newnr < 100)) ? 1 : 0;
            ...
            ok = (knoop.newnr === -1) ? 1 : 0;
            newstat = "DEL";
        }

        // "FID", "code", "id", "oldnr", "newnr", "oldstatus", "newstatus", "change", "ok"
        knopen.push([knoop.FID, knoop.code, knoop.id,
                    knoop.oldnr, knoop.newnr,
                    knoop.status, newstat, change, ok]);
    });

    /*-------------------------------  result list of processed links ---------*/
    Object.keys(linkByCode).forEach(function (lnkcode) {
        var link = linkByCode[lnkcode], minnr = -1, maxnr = 100,
            startcode = "#N/A", endcode = "#N/A",
            newstat = "UNKNOWN", change, ok = 0;

        function tgtOk(tgt) {
            if (!contains('FIX', 'FIXNEW', 'IN', 'NEW')) {
                return true; // since we don't care
            }
            //else
            return (!isNaN(tgt.newnr) && (tgt.newnr < 100) && tgt.newnr > 0);
        }
        ...
        if (link.status === 'NIV') {
            ...
            minnr = Math.min(link.start.newnr, link.end.newnr);
            maxnr = Math.max(link.start.newnr, link.end.newnr);
            change = ((link.start.oldnr === link.start.newnr) && (link.end.oldnr === link.end.newnr)) ? 0 : 1;
            ok = (tgtOk(link.start) && tgtOk(link.end)) ? 1 : 0;
            startcode = link.start.code;
            endcode = link.end.code;

            if (contains(['NEW', 'FIX', 'MIS'], link.status)) {
                newstat = "FIX";
            } else if (link.status === 'DEL') {
                newstat = "DEL";
            }
        }


        //"FID", "pid", "code", "startcode", "endcode", "startnr", "endnr", "oldstatus", "newstatus", "change", "ok"
        links.push([ link.FID, link.pid, link.code, startcode, endcode, minnr, maxnr,
                    link.status, newstat, change, ok]);
    });
    ...
    /*-------------------------------  result statistics           ----------*/
    for (nr = 1; nr < 100; nr += 1) {
        nrset = this.usage.knoopbynr[nr];
        cnt = nrset ? nrset.length : 0;

        minDist = (nrset && nrset.minDist) ? Math.round(nrset.minDist * 100) / 100 : " N/A ";
        minDistOwn = (nrset && nrset.minDistOwn) ? Math.round(nrset.minDistOwn * 100) / 100 : " N/A ";
        friends = Object.keys(this.usage.linkbynr[nr]);

        // "nr", "count", "dist", "spare_links"
        stats.push([nr, cnt, minDist, minDistOwn, (100 - friends.length)]);
    }

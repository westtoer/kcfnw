...
function Input(conf) {
    ...
    console.log("cutoff test '%s' (%d) '%s' (%d)", this.cutoffcode, this.cutoffval, value2Code(this.cutoffval), code2Value(this.cutoffcode));
    this.maxCodeValue = 0;
    this.knoopByCode = {};
    this.linkByCode = {};
    this.err = {
...
Input.prototype.reportCounts = function () {
    ...
    function addToActionSet(action, knoop) {
        ...
        if (knoop.code === 'R9') { console.log("adding R9 to %s: %s, %s", action, knoop, knoop.mustChange);  }

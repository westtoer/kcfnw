...
WorkSet.prototype.position = function(type){
    ...
    function logpos( lid,ndx,trcnt,trndx,trclen, prev,current,comment ) {
        console.log("\t\t%s,%s,%s,%s,%s,%s,%s,%s", lid,ndx,trcnt,trndx,trclen,prev,current,comment);
    }
        ...
        line.knopen.every( function( nr, index){
            ...
            if (index == 0) { // at the start we launch as many traces as positioned start-knopen
                if (!positionedKnopen || positionedKnopen.length == 0){
                    logpos(line.id, index, -1, -1, -1, "#", nr, "no trace started");
                    return false;
                }

                logpos(line.id, index, positionedKnopen.length, -1 , -1, "#", nr, "INIT - initial positions found");
                ...
                logpos(line.id, index, traces.length, -1, -1, "#", nr, "traces started");
                ...
                logpos(line.id, index, traces.length, -1, -1, "#", nr,
                       "NEXT - done evaulating traces at this index");
            }
            return true;
        });

...
BaseData.prototype.statsCSV = function (outDir) {
    ...
    Object.keys(this.data).forEach(function (type) {
        ...
        rwcsv.write(path.join(outDir, csvName), knoopStatsData,
                     ["id", "cnt", "avg", "min",
...
BaseData.prototype.linkKey = function (net, nr1, nr2) {
    ...
    if (nr2 < nr1) { h = nr1; nr1 = nr2; nr2 = h; }
...
BaseData.prototype.addKnoop = function (data) {
    ...
    var knoop = {
        ...
        knoopKEY, exists, knopenAtKey, dist;
...
WorkSet.prototype.positionLogCSV = function (outDir) {
    rwcsv.write(path.join(outDir, "dbglog-positioning.csv"), this.positionLog,
...
ResultSet.prototype.to = function (outDir) {
    ...
    rwcsv.write(path.join(outDir, "knoop-use.csv"), this.knoopStatsById,
                 ["id", "lbl", "usecnt", "wkt"]);

    rwcsv.write(path.join(outDir, "link-use.csv"), this.linkStatsById,
                 ["id", "pid", "lbl", "start", "end", "dist", "usecnt", "wkt"]);

    rwcsv.write(path.join(outDir, "line-report.csv"), this.lineStatsById,
                 ["id", "hops", "dist", "lus"]);
};

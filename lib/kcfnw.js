...
WorkSet.prototype.position = function(){
    ...
    var allAmbiguLines = 0;
    var biggestAmbiguLine = 0;
    this.lines.forEach(function(line){
        //position the knopen in this line
        var netwerkid = line.netwerkid;
        var allAmbigu = true; // assume the worst
        line.positions = [];
        line.knopen.forEach(function(nr){
            var positions = base.knoopByKey[keyJoin(netwerkid, nr)];
            line.positions.push(positions);
            var hits = (!positions) ? 0 : positions.length;
            duplicates[hits]++;
            allAmbigu = allAmbigu && (hits != 1);
        });
        if (allAmbigu) {
            allAmbiguLines++;
            biggestAmbiguLine = Math.max(biggestAmbiguLine, line.knopen.length);
        }
    });
    console.log("all lines in enq cnt ==> %d", this.lines.length);
    console.log("all ambigu lines cnt ==> %d", allAmbiguLines);
    console.log("biggest ambigu line  ==> %d", biggestAmbiguLine);

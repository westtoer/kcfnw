...
WorkSet.prototype.execute = function() {
    ...
    self.prepare(function(ok, err) {
        ...
        self.readLines(function(){
            self.position(type);
            self.countUse();
            self.results.toTables();
            self.results.toPlots();
        });
    });
}
...
ResultSet.prototype.countKnoop = function(knoop) {
    ...
    if (knoopStats == null ) { // not yet listed - first count for knoop
...
ResultSet.prototype.countLink = function(link) {
    ...
    if (linkStats == null ) { // not yet listed - first count for link
...
ResultSet.prototype.toTables = function(){
    console.log("toTables--knoopuse");
    console.log("id,lbl,usecnt,wkt");
    var knStats = this.knoopStatsById;
    Object.keys(knStats).forEach(function(knpid){
        var kn = knStats[knpid];
        console.log("%s,%s,%s,%s", kn.id, kn.lbl, kn.usecnt, kn.wkt);
    });

    console.log("toTables--linkuse");
    console.log("id,lbl,start,end,dist,usecnt,wkt");
    var lkStats = this.linkStatsById;
    Object.keys(lkStats).forEach(function(lnkid){
        var lk = lkStats[lnkid];
        console.log("%s,%s,%s,%s,%s,%s,%s", lk.id, lk.lbl, lk.start, lk.end, lk.dist, lk.usecnt, lk.wkt);
    });

    console.log("toTables--linereport");
    console.log("id,hops,dist,lus");
    var lnStats = this.lineStatsById;
    Object.keys(lnStats).forEach(function(linid){
        var ln = lnStats[linid];
        console.log("%s,%s,%s,%s", ln.id, ln.hops, ln.dist, ln.lus);
    });
};

ResultSet.prototype.toPlots = function(){
    console.log("toPlots");
    /*
    var doc = xb.create();
    doc.begin('kml')
        ...
        .ele('Document')
            .ele('Name')
    	<Folder>
		<name>kc-fnw-enq</name>
		<open>1</open>
		<Folder>
			<name>heatmap</name>
			<Placemark>
				<name>10-16</name>
				<description>aantal: 107</description>
				<styleUrl>#msn_ylw-pushpin0</styleUrl>
				<LineString>
					<tessellate>1</tessellate>
					<coordinates>
						2.957906330875697,51.24656797652298,0 2.963068270414888,51.23919580145302,0 2.960117204851092,51.23872156031282,0 2.961280966330655,51.23501912066657,0
					</coordinates>
				</LineString>
			</Placemark>
			<Placemark>
				<name>10-13</name>
				<description>aantal: 43</description>
				<styleUrl>#msn_ylw-pushpin</styleUrl>
				<LineString>
					<tessellate>1</tessellate>
					<coordinates>
						2.964213333454135,51.24668460533152,0 2.966062379544296,51.24748848011918,0 2.965151709783895,51.24920656022979,0 2.963849761291839,51.24862861241695,0 2.961766392585707,51.24789571937749,0 2.957909108415255,51.2465654579662,0
					</coordinates>
				</LineString>
			</Placemark>
		</Folder>
		<Folder>
			...
			<Folder>
				...
				<Placemark>
					<name>10</name>
					<description>aantal: 23</description>
					<LookAt>
						<longitude>2.964159763801921</longitude>
						<latitude>51.24222961874023</latitude>
						<altitude>0</altitude>
						<heading>-4.376403679411746</heading>
						<tilt>16.9464187633346</tilt>
						<range>5063.693756606309</range>
						<gx:altitudeMode>relativeToSeaFloor</gx:altitudeMode>
					</LookAt>
					<styleUrl>#m_ylw-pushpin</styleUrl>
					<Point>
						<gx:drawOrder>1</gx:drawOrder>
						<coordinates>2.958242626716452,51.24636127682129,0</coordinates>
					</Point>
				</Placemark>
*/
};

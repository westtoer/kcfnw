...
- cli-switch om netwerk-type in te geven WANDEL of FIETS zodat we toch niet alles meenemen
...
- alternatief: werken met koppeling start-gemeente >> newterkid zodat we buiten het standardnet kunnen zoeken voor de start
        ...
        line.knopen.every( function( nr, index){
            ...
            if (index == 0) { // at the start we launch as many traces as positioned start-knopen
                if (!positionedKnopen || positionedKnopen.length == 0){
                    console.log("Geen traces gelanceerd voor lijn %d startend met nr [%d]",
                                line.id, nr);
                    return false;
                } else {
                    console.log("Startend met '%d' traces op lijn %d voor nr [%d]",

...
WorkSet.prototype.execute = function() {
    ...
    self.prepare(function(ok, err) {
        ...
        self.base.statsCSV(self.outDirPath, type);
...
WorkSet.prototype.positionLogCSV = function(outDir) {
    writeCSVFile(path.join(outDir,"dbglog-positioning.csv"), this.positionLog,
        ["line_id", "index_in_line", "active_traces", "current_trace_index", "current_trace_length",
...
ResultSet.prototype.to = function(outDir){
    ...
    writeCSVFile(path.join(outDir,"knoop-use.csv"), this.knoopStatsById,
                 ["id", "lbl", "usecnt", "wkt"]);

    writeCSVFile(path.join(outDir,"link-use.csv"), this.linkStatsById,
                 ["id", "pid", "lbl", "start", "end", "dist", "usecnt", "wkt"]);

    writeCSVFile(path.join(outDir,"line-report.csv"), this.lineStatsById,
                 ["id", "hops", "dist", "lus"]);
};
...
BaseData.prototype.statsCSV = function(outDir, type) {
    if (!type) {
        console.log("No CSV output for base data since no type was specified");
        return;
    }
    ...
    writeCSVFile(path.join(outDir,"base-stats-knopen.csv"), knoopStatsData,
                 ["id","cnt","avg","min","minatlat1","minatlon1","minatlat2","minatlon2"]);
}
...
function writeCSVFile(csvPath, data, fields) {
    ...
    var csvFile = fs.createWriteStream(csvPath, { flags: 'w', encoding: 'utf-8', mode: 0666 });
    csvFile.on('error',function(err){console.error(err)});
    ...
    if (typeOfData == '[object Object]') {
        out = csv.stringify({columns: fields, header: true});
